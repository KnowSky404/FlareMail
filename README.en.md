...
- Explicit `AUTH_MODE=local|cloudflare-access` authentication: a non-email local username and PBKDF2 password, or a verified Cloudflare Access identity mapped to the same stable Owner. Both modes retain revocable D1 sessions, secure cookies, Origin/CSRF validation, login rate limiting, and security headers. When Access expires, the browser keeps the active compose buffer in that tab and refreshes only safe GETs after reauthentication; it never blindly replays writes.
...
- Multiple sender identities: new mail prefers a ready exact address selected by the current address filter, while other views use the ready global default; replies and drafts keep their original identity. The server checks Owner, address state, and the domain's Resend sending verification, then freezes the From/signature snapshot. The compose UI explains disabled, unverified, stale, and failed sender states. Legacy `OUTBOUND_FROM_EMAIL` / `MAIL_FROM` values are only for system auto-replies and notifications.
