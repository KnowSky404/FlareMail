...
- 邮件身份可靠性：当前视图批量操作的服务端范围约束、地址路由删除指纹/对账/恢复租约、schema 25 的 Cloudflare/Resend 独立只读续检、schema 26 的删除预览与三种删除策略，以及仅本地运行的历史地址 plan/apply/verify。仍需在独立环境验证调度、provider 与经授权的生产回填；`docs/PRODUCTION_CHECKLIST.md` 保留相应门禁。
...
- `scheduled()` maintenance：Telegram outbox 与 Cloudflare/Resend 健康续检独立隔离；domain check 使用 due 索引、短租约和有界工作。R2/outbox/stale-claim/session/FTS/trash/orphan/audit retention 等后续 maintenance 项仍未全部接入；破坏操作默认 dry-run。
