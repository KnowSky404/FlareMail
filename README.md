...
- 版本化 D1 migration：`migrations/0001` 至 `0025`。`0023` 将稳定 Owner 与本地用户名/凭据分开，`0024` 添加受管域名/地址和邮件身份快照，`0025` 添加独立的 Cloudflare/Resend 健康续检租约；历史邮件不会因资料邮箱或旧登录邮箱自动变成可收发地址。`schema.sql` 是最新结构快照。
