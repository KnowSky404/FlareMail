...
<div class={cn('flex items-start gap-3 rounded-[var(--radius-md)] border px-3 py-2.5 text-sm', toneClasses[variant], className)} role={variant === 'danger' ? 'alert' : 'status'}>
  ...
  {#if dismissible}<button class="fm-touch-target grid size-8 place-items-center rounded opacity-70 hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current" aria-label={t('common.close')} type="button" onclick={onDismiss}><X class="size-4" aria-hidden="true" /></button>{/if}
</div>
