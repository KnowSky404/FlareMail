  ...
  <div class="flex items-center gap-1 border-b border-[var(--fm-border)]" role="tablist" aria-label={t('common.tabs')}>
    {#each tabs as tab, index (tab.id)}
      <button
        ...
        class={cn('fm-touch-target relative px-3 py-2.5 text-sm font-medium text-[var(--fm-text-muted)] transition-colors hover:text-[var(--fm-text)] disabled:cursor-not-allowed disabled:opacity-50', focusRing, activeValue === tab.id && 'text-[var(--fm-text)] after:absolute after:inset-x-0 after:-bottom-px after:h-0.5 after:bg-[var(--fm-primary)]')}
