...
function shouldSkipAutoReply(message: ForwardableEmailMessage) {
  ...
  const suppress = message.headers.get('x-auto-response-suppress')?.toLowerCase() ?? '';
  const autoSubmittedBlocks = autoSubmitted !== '' && autoSubmitted !== 'no';
  const sender = message.from.trim().toLowerCase();
  return autoSubmittedBlocks || ['bulk', 'list', 'junk'].includes(precedence ?? '') || Boolean(message.headers.get('list-id')?.trim()) ||
    blockedLocalParts.has(localPart) || !sender || !sender.includes('@') ||
    /^(all|dr|rn|nrn|auto-reply|autoreply|noreply)$/iu.test(suppress);
}
...
export async function sendAutomaticReply(message: ForwardableEmailMessage, env: CloudflareEnv, storageId: string) {
  if (!parseBoolean(env.AUTO_REPLY_ENABLED)) return { sent: false, reason: 'AUTO_REPLY_ENABLED is disabled.' };
  if (!isValidEmail(message.from) || shouldSkipAutoReply(message)) return { sent: false, reason: 'Auto-reply loop guard matched.' };
...
export async function sendInboundNotification(env: CloudflareEnv, input: {
  ...
  if (!parseBoolean(env.INBOUND_NOTIFICATION_ENABLED)) return { sent: false, reason: 'INBOUND_NOTIFICATION_ENABLED is disabled.' };
