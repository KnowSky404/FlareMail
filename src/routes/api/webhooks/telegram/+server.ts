...
function webhookJson(body: Record<string, unknown>, init: ResponseInit = {}) {
...
export const POST: RequestHandler = async (event) => {
  ...
  if (!configuredSecret || !constantTimeEqual(configuredSecret, suppliedSecret)) {
    return webhookJson({ ok: false, error: 'Webhook verification failed.' }, { status: 401 });
  }

  const config = resolveTelegramConfig(env);
  if (!config.enabled) return webhookJson({ ok: false, code: 'TELEGRAM_DISABLED' }, { status: 404 });
  if (!config.ready || !env?.DB) return webhookJson({ ok: false, code: 'TELEGRAM_NOT_READY' }, { status: 503 });
  ...
  if (Number.isFinite(declaredLength) && declaredLength > _maxTelegramWebhookBodyBytes) {
    return webhookJson({ ok: false, code: 'WEBHOOK_BODY_TOO_LARGE' }, { status: 413 });
  }
  let body: string;
  try {
    body = await readBoundedBody(event.request);
  } catch (error) {
    if (error instanceof TelegramWebhookBodyTooLargeError) return webhookJson({ ok: false, code: 'WEBHOOK_BODY_TOO_LARGE' }, { status: 413 });
    return webhookJson({ ok: false, code: 'WEBHOOK_BODY_INVALID' }, { status: 400 });
  }
  ...
  try {
    ...
    return webhookJson({ ok: false, code: 'INVALID_UPDATE' }, { status: 400 });
  }
  const updateId = normalizeTelegramId(update.update_id);
  if (!updateId) return webhookJson({ ok: false, code: 'INVALID_UPDATE_ID' }, { status: 400 });
  if (!await hasTelegramTables(env.DB)) return webhookJson({ ok: false, code: 'SCHEMA_NOT_READY' }, { status: 503 });
  ...
  const isPrivateUserMessage = Boolean(
    message && chatId && userId && chatId === userId && message.chat?.type === 'private' && message.from?.is_bot === false &&
    isAbsent(message.forward_origin) && isAbsent(message.forward_from) && isAbsent(message.forward_from_chat) &&
    isAbsent(message.via_bot) && isAbsent(message.sender_chat) && message.is_automatic_forward !== true
  );
  if (!isPrivateUserMessage) {
    try {
      await recordTelegramIgnoredUpdate(env.DB, updateId, 'not_private_user_message');
    } catch {
      return webhookJson({ ok: false, code: 'TEMPORARY_DATABASE_FAILURE' }, { status: 503 });
    }
  ...
  if (!command) {
    try {
      await recordTelegramIgnoredUpdate(env.DB, updateId, 'ignored_message');
    } catch {
      return webhookJson({ ok: false, code: 'TEMPORARY_DATABASE_FAILURE' }, { status: 503 });
    }
    return webhookJson({ ok: true });
  ...
  if (command.command === 'help' || (command.command === 'start' && !command.argument)) {
    let recorded = false;
    try {
      recorded = await recordTelegramIgnoredUpdate(env.DB, updateId, 'help');
    } catch {
      return webhookJson({ ok: false, code: 'TEMPORARY_DATABASE_FAILURE' }, { status: 503 });
    }
    if (recorded) {
  ...
  if (command.command === 'stop') {
    let stop: { duplicate: boolean; revoked: boolean };
    try {
      stop = await processTelegramStopUpdate(env.DB, { updateId, telegramChatId: chatId! });
    } catch {
      return webhookJson({ ok: false, code: 'TEMPORARY_DATABASE_FAILURE' }, { status: 503 });
    }
    if (!stop.duplicate) {
      const ctx = event.platform?.context ?? event.platform?.ctx;
      ctx?.waitUntil(sendBestEffort(env, chatId!, stop.revoked ? 'FlareMail Telegram 通知已停用并解除绑定。' : '当前聊天没有已绑定的 FlareMail 账号。'));
    }
    return webhookJson({ ok: true });
  }
  ...
  if (!/^[A-Za-z0-9_-]{32,64}$/u.test(challengeToken)) {
    try {
      await recordTelegramIgnoredUpdate(env.DB, updateId, 'invalid_challenge_token');
    } catch {
      return webhookJson({ ok: false, code: 'TEMPORARY_DATABASE_FAILURE' }, { status: 503 });
    }
    return webhookJson({ ok: true });
  ...
  let outcome: Awaited<ReturnType<typeof consumeTelegramChallenge>>;
  try {
    outcome = await consumeTelegramChallenge(env.DB, {
      updateId,
      processingToken,
